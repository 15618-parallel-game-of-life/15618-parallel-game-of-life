...
class Network:
  ...
  def copy_and_mutate(self, network, mr=0.1):
    ...
    self.weights_1 = self.weights_1 / np.linalg.norm(self.weights_1)
    self.weights_2 = self.weights_2 / np.linalg.norm(self.weights_2)
    self.weights_3 = self.weights_3 / np.linalg.norm(self.weights_3)
    self.weights_4 = self.weights_4 / np.linalg.norm(self.weights_4)
    self.biases_in = self.biases_in / np.linalg.norm(self.biases_in)
    self.biases_out = self.biases_out / np.linalg.norm(self.biases_out)
...
for episode in range(1, episodes+1):
  ...
  with multiprocessing.Pool() as pool:
    scores = pool.map(selfPlay, models)
